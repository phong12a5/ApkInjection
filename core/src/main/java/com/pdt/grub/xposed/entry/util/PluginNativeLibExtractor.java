package com.pdt.grub.xposed.entry.util;
...
public class PluginNativeLibExtractor {
    ...
    public static void copySoFileIfNeeded(Context context, String libPath, String pluginApkPath) {
        ...
        if (abiSet.isEmpty()) {
            PLog.i(TAG, " plugin: " + pluginApkPath + " do not contains any so files.");
            return;
        }

        XpatchUtils.ensurePathExist(libPath);

        PLog.i(TAG, " copySoFileIfNeeded procecess = " + XpatchUtils.getCurProcessName(context) + " isMainProcess = " + XpatchUtils.isMainProcess(context));
        PLog.i(TAG, " copyPluginSoFile libPath = " + libPath + " pluginApkPath = " + pluginApkPath);
        ...
        PLog.i(TAG, " copyPluginSoFile savedMd5 = " + savedMd5 + " curMd5 = " + curMd5);
        if (savedMd5 == null || savedMd5.isEmpty() || !savedMd5.equals(curMd5)) {
            ...
            PLog.d(TAG, "plugin is not changed, no need to copy so file again!");
        }
    }
